...
export function AnalysisHeader({ conversation, dataset, profileComplete, result, onRename, onDelete, onOpenQuestions }) {
  ...
  return (
    <header className="border-b border-[#E5E5EA] bg-white px-3 py-2 sm:px-5 sm:py-2.5">
      <div className="flex min-w-0 items-center justify-between gap-2 sm:gap-4">
        <div className="min-w-0 flex-1">
          ...
          <p className="mb-0 mt-0.5 flex min-w-0 items-center gap-1.5 text-[10px] leading-4 text-[#6E6E73] sm:text-[11px]">
        ...
        <div className="flex shrink-0 items-center gap-0.5 sm:gap-1">
          <button className="inline-flex h-9 items-center justify-center gap-1.5 rounded-lg border-0 bg-transparent px-2 text-[#6E6E73] hover:bg-[#F5F5F7] hover:text-[#1D1D1F] lg:hidden" type="button" onClick={onOpenQuestions} aria-label="Open questions" title="Questions"><QuestionsIcon /><span className="hidden text-[11px] font-medium md:inline">Questions</span></button>
          {result?.report && <button className="inline-flex h-9 items-center justify-center gap-1.5 rounded-lg border-0 bg-transparent px-2 text-[#6E6E73] hover:bg-[#F5F5F7] hover:text-[#1D1D1F] disabled:opacity-50" type="button" onClick={openPreview} disabled={downloading} aria-label={downloading ? 'Preparing PDF' : 'Preview PDF'} title={downloading ? 'Preparing PDF…' : 'Preview PDF'}>{downloading ? <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-[#C7C7CC] border-t-[#515154]" /> : <PdfIcon />}<span className="hidden text-[11px] font-medium md:inline">{downloading ? 'Preparing…' : 'PDF'}</span></button>}
...
function QuestionsIcon() {
  return <svg className="h-4 w-4" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true"><path d="M4.25 4.25h11.5v8.5H8l-3.75 3v-11.5Z" /><path d="M7 7.25h6M7 9.75h4" /></svg>;
}

function PdfIcon() {
  return <svg className="h-4 w-4" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true"><path d="M5 2.75h6l4 4v10.5H5V2.75Z" /><path d="M11 2.75v4h4M7.25 13.5h5.5M7.25 10.75h5.5" /></svg>;
}
