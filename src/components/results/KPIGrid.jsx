  ...
  return (
    <section aria-label="Key performance indicators">
      <div className="grid grid-cols-2 gap-2.5 sm:gap-3 lg:grid-cols-3 2xl:grid-cols-4">
        {kpis.map((kpi, index) => (
          <article className={`min-w-0 rounded-xl border border-[#E5E5EA] bg-white px-3 py-3 shadow-[0_2px_10px_rgba(0,0,0,0.025)] sm:px-4 sm:py-3.5 ${kpis.length % 2 === 1 && index === kpis.length - 1 ? 'col-span-2 sm:col-span-1' : ''}`} key={kpi.title} title={kpi.exactValue}>
