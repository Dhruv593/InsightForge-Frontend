...
export function SupportingVisuals({ charts = [] }) {
  const visibleCharts = charts.slice(0, 4);
    ...
    <section aria-labelledby="supporting-visuals">
      ...
      <div className="grid gap-4 2xl:grid-cols-2">
        {visibleCharts.map((chart, index) => (
          <Suspense fallback={<ChartSkeleton className={visibleCharts.length === 1 || (visibleCharts.length === 3 && index === 2) ? '2xl:col-span-2' : ''} />} key={chart.id}>
            <ChartRenderer chart={chart} className={visibleCharts.length === 1 || (visibleCharts.length === 3 && index === 2) ? '2xl:col-span-2' : ''} />
          </Suspense>
        ))}
      </div>
    </section>
