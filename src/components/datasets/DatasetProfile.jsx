export function DatasetProfile({ profile, loading, profiling, onProfile }) {
  ...
  return (
    <section className="rounded-xl border border-[#E5E5E8] bg-white p-5 sm:p-7">
      ...
      <dl className="my-6 grid grid-cols-2 border-y border-slate-200 sm:grid-cols-4">{[['Rows', profile.row_count?.toLocaleString()], ['Columns', profile.column_count], ['Missing cells', missing.toLocaleString()], ['Duplicate rows', duplicates.toLocaleString()]].map(([label, value], index) => <div className={`p-4 ${index === 0 ? 'pl-0' : index === 2 ? 'pl-0 sm:pl-4' : ''} ${index < 3 ? 'sm:border-r sm:border-slate-200' : ''}`} key={label}><dt className="text-xs text-slate-500">{label}</dt><dd className="mb-0 mt-1.5 text-xl font-semibold text-slate-900">{value}</dd></div>)}</dl>
