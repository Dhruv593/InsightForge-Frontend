  ...
  return (
    <form className="query-composer shrink-0 bg-transparent px-2.5 pt-2.5 sm:px-6 sm:pt-3" onSubmit={submit}>
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mt-1.5 flex min-h-4 flex-col items-center justify-center gap-x-2 px-3 text-center text-[9px] leading-4 sm:flex-row">
          <span id="analysis-query-help" className={`text-[#8E8E93] transition-opacity ${queryFocused && !disabled && !profileRequired && !insufficientCredits ? 'opacity-100' : 'hidden opacity-0 sm:inline'}`}>Enter to send · Shift + Enter for a new line</span>
          <span id="analysis-ai-note" className="text-[#A1A1AA]">Tatparya uses AI and may make mistakes. Verify important results.</span>
        </div>
      </div>
    </form>
  );
