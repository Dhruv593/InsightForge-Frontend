...
export function QueryBox({ disabled, submitting, error, profileRequired, insufficientCredits, onProfile, onSubmit, draft }) {
  ...
  async function submit(event) {
    ...
    let succeeded = false;
    try {
      succeeded = await onSubmit({ query: normalized });
    } finally {
      if (!succeeded) {
        setQuery(normalized);
        window.requestAnimationFrame(() => queryRef.current?.focus());
  ...
  return (
    <form className="query-composer w-full min-w-0 max-w-full shrink-0 overflow-x-hidden bg-transparent px-2.5 pt-2 sm:px-6 sm:pt-3" onSubmit={submit}>
