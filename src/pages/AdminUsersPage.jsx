...
export function AdminUsersPage() {
  ...
  return <AdminShell><div className="mx-auto grid max-w-5xl gap-6">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="m-0 text-2xl font-semibold tracking-[-0.035em] text-[#1D1D1F] sm:text-3xl">Users & access</h2><p className="mb-0 mt-2 text-sm leading-6 text-[#6E6E73]">Review verification status and control who can access the admin dashboard.</p></div><input className="min-h-11 w-full rounded-xl border border-[#D2D2D7] bg-white px-3 text-sm outline-none focus:border-brand-500 sm:w-64" type="search" placeholder="Search name or email" value={search} onChange={(event) => setSearch(event.target.value)} /></header>
...
function CreditEditor({ name, value, saving, onAdjust }) {
  ...
  return <div className="grid gap-2">
    <input className="h-9 w-full rounded-lg border border-[#DADAE0] bg-white px-2 text-[11px] outline-none focus:border-brand-500 disabled:bg-[#F7F7F8] sm:w-44" type="text" maxLength="240" placeholder="Reason for credit change" aria-label={`Reason for changing ${name}'s credits`} value={reason} disabled={saving} onChange={(event) => setReason(event.target.value)} />
