...
export function AdminPlansPage() {
  ...
  return <AdminShell><div className="mx-auto grid max-w-5xl gap-6">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="m-0 text-2xl font-semibold tracking-[-0.035em] text-[#1D1D1F] sm:text-3xl">Plans page</h2><p className="mb-0 mt-2 text-sm leading-6 text-[#6E6E73]">Control whether pricing is visible and manage each credit package.</p></div><div className="grid grid-cols-2 gap-2 sm:flex"><Link className="inline-flex min-h-11 items-center justify-center rounded-lg border border-[#D2D2D7] bg-white px-3 text-xs font-semibold text-[#3A3A3C]" to="/plans" target="_blank">View page ↗</Link><button className="min-h-11 rounded-lg bg-brand-600 px-3 text-xs font-semibold text-white disabled:opacity-50" type="submit" form="plans-form" disabled={loading || saving}>{saving ? 'Publishing…' : 'Publish changes'}</button></div></header>
