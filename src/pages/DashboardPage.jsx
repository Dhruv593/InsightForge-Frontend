...
function NewAnalysisFields({ datasets, profiles, title, datasetId, uploading, onTitleChange, onDatasetChange, onUpload }) {
  ...
  return (
    <div className="grid gap-5">
      ...
      {datasets.length ? <div className="grid gap-2"><label className="text-sm font-medium text-[#3A3A3C]" htmlFor="analysis-dataset">Dataset</label><select className="min-h-11 w-full rounded-lg border border-[#D2D2D7] bg-white px-3 py-2 text-[13px] text-[#1D1D1F] transition focus:border-brand-500" id="analysis-dataset" value={datasetId} onChange={(event) => onDatasetChange(event.target.value)}>{datasets.map((dataset) => <option value={dataset.id} key={dataset.id}>{dataset.original_file_name} — {datasetMeta(dataset, profiles?.[dataset.id])}</option>)}</select>{selected && <p className="m-0 text-xs text-[#6E6E73]">{datasetMeta(selected, profiles?.[selected.id])}</p>}</div> : <div className="rounded-xl bg-[#F5F5F7] p-4"><p className="m-0 text-sm font-medium text-[#3A3A3C]">No datasets uploaded yet.</p><p className="mb-4 mt-1 text-xs leading-5 text-[#6E6E73]">Upload a dataset before starting an analysis.</p><button className="inline-flex min-h-9 items-center rounded-lg border border-[#D2D2D7] bg-white px-3 text-xs font-medium text-[#3A3A3C] hover:bg-[#FAFAFB] disabled:opacity-50" type="button" onClick={onUpload} disabled={uploading}>{uploading ? 'Uploading…' : 'Upload Dataset'}</button></div>}
    </div>
  );
}
